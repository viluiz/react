...
for j in range(runs):
    ...
    for i, line in enumerate(lines):
        # find Ca
        if 'Ca ' in line:
            aux = str(np.random.uniform(Ca_range[0],Ca_range[1]))
            lines[i] = re.sub(line.split()[1], aux, line)
        # find Cl
        if 'Cl ' in line:
            aux = str(np.random.uniform(Cl_range[0],Cl_range[1]))
            lines[i] = re.sub(line.split()[1], aux, line)
        # find Na
        if 'Na ' in line:
            aux = str(np.random.uniform(Na_range[0],Na_range[1]))
            lines[i] = re.sub(line.split()[1], aux, line)
        # find K
        if 'K ' in line:
            aux = str(np.random.uniform(K_range[0],K_range[1]))
            lines[i] = re.sub(line.split()[1], aux, line)
        # find N(5)
        if 'N(5) ' in line:
            aux = str(np.random.uniform(N5_range[0],N5_range[1]))
            lines[i] = re.sub(line.split()[1], aux, line)
